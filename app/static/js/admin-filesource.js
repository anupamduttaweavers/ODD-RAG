...
async function loadSources() {
    try {
        var res = await ADMIN_AUTH.adminFetch(API + "/");
...
function openModal(title) { document.getElementById("modalTitle").textContent = title; document.getElementById("modalOverlay").classList.add("open"); }
function closeModal() {
    ...
    editingId = null; updateProtocolFields();
}
...
function openAdd() {
    ...
    openModal("Add File Source"); updateProtocolFields();
}
...
function openEdit(id) {
    ...
    openModal("Edit File Source"); updateProtocolFields();
}
...
async function saveSource() {
    ...
    if (!data.name || !data.base_path) { showToast("Name and Base Path are required", "error"); return; }
    var url = editingId ? API + "/" + editingId : API + "/";
    var method = editingId ? "PUT" : "POST";
    try {
        var res = await ADMIN_AUTH.adminFetch(url, { method: method, body: JSON.stringify(data) });
        var body = await res.json();
        if (body.success) { showToast(body.message || "Saved", "success"); closeModal(); loadSources(); }
        else { showToast(body.message || body.error_message || "Save failed", "error"); }
    } catch (e) { showToast("Request failed: " + e.message, "error"); }
}

async function toggleEnable(id, enabled) {
    try { await ADMIN_AUTH.adminFetch(API + "/" + id, { method: "PUT", body: JSON.stringify({ is_enabled: enabled }) }); loadSources(); }
    catch (e) { showToast("Toggle failed", "error"); }
}

function confirmDelete(id, name) { if (confirm("Delete source '" + name + "'? This cannot be undone.")) deleteSource(id); }
async function deleteSource(id) {
    try {
        var res = await ADMIN_AUTH.adminFetch(API + "/" + id, { method: "DELETE" });
        var body = await res.json();
        showToast(body.message || "Deleted", body.success ? "success" : "error"); loadSources();
...
async function testConn(id) {
    ...
    try {
        var res = await ADMIN_AUTH.adminFetch(API + "/" + id + "/test", { method: "POST" });
        var body = await res.json();
        showToast((body.source_name || "") + ": " + body.message, body.success ? "success" : "error"); loadSources();
...
async function testFromModal() {
    ...
    var box = document.getElementById("testResultBox"); box.style.display = "block"; box.textContent = "Testing...";
    try {
        var res = await ADMIN_AUTH.adminFetch(API + "/" + editingId + "/test", { method: "POST" });
        var body = await res.json();
        box.textContent = JSON.stringify(body, null, 2);
        showToast(body.message || (body.success ? "OK" : "Failed"), body.success ? "success" : "error"); loadSources();
...
async function pullFiles(id) {
    ...
    try {
        var res = await ADMIN_AUTH.adminFetch(API + "/" + id + "/pull", { method: "POST" });
        var body = await res.json();
        showToast((body.source_name || "") + ": " + body.message, body.success ? "success" : "error"); loadSources();
...
async function scanSource(id) {
    ...
    try {
        var res = await ADMIN_AUTH.adminFetch(API + "/" + id + "/scan", { method: "POST" });
        var body = await res.json();
        showToast((body.source_name || "") + ": " + body.message, body.success ? "success" : "error"); loadSources();
